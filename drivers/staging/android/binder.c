	...
	for (; offp < off_end; offp++) {
		...
		if (object_size == 0 || *offp < off_min) {
			binder_user_error("%d:%d got transaction with invalid offset, %lld (min %zd, max %zd)\n",
                               proc->pid, thread->pid, (u64)*offp, off_min,
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BINDER_WRITE_READ: {
		...
		binder_debug(BINDER_DEBUG_READ_WRITE,
			     "binder: %d:%d write %lld at %016llx, read %lld at %016llx\n",
			     proc->pid, thread->pid, (u64)bwr.write_size, (u64)bwr.write_buffer,
